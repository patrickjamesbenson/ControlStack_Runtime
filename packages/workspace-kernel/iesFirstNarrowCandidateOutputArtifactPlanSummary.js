...
export const RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_CONTRACT_ID = "RUNTIME-IES-FIRST-NARROW-CANDIDATE-OUTPUT-ARTIFACT-PLAN-WRITE-1";
export const RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_ID = "controlstack.runtime.ies-first-narrow-candidate-output-artifact-plan-summary.v1";
export const RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_VERSION = 1;
export const RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_TARGET = "projectEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactPlanSummary";

export const RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_REQUIRED_FALSE_FLAGS = Object.freeze([
  ...RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_DETAIL_REQUIRED_FALSE_FLAGS,
  "artifactPlanGenerationEnabled",
  "artifactPlanGenerationAttempted",
  "artifactPlanGenerated",
  "artifactPlanFileOutputEnabled",
  "artifactPlanFileOutputWritten",
]);

export const RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_FIELD_ORDER = Object.freeze([
  "schemaId",
  "schemaVersion",
  "contractId",
  "owner",
...
const READY_STATE = "redacted_ies_first_narrow_candidate_output_artifact_plan_summary_persisted";
...
export function findUnsafeIesFirstNarrowCandidateOutputArtifactPlanInput(value, depth = 0, seen = new Set()) {
  ...
  if (typeof value === "string") {
    if (PRIVATE_VALUE_PATTERN.test(value) || RAW_IES_TEXT_PATTERN.test(value) || DATA_BASE64_PATTERN.test(value) || FILE_EXTENSION_VALUE_PATTERN.test(value)) {
      return "ies-first-narrow-candidate-output-artifact-plan-unsafe-string";
    }
    return null;
  }
  if (Array.isArray(value)) {
    for (const item of value.slice(0, 80)) {
      const nested = findUnsafeIesFirstNarrowCandidateOutputArtifactPlanInput(item, depth + 1, seen);
  ...
  for (const [key, nested] of Object.entries(value)) {
    ...
    const child = findUnsafeIesFirstNarrowCandidateOutputArtifactPlanInput(nested, depth + 1, seen);
...
function orderedSummary(fields) {
  return Object.fromEntries(RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_FIELD_ORDER.map((key) => [key, fields[key]]));
}

function disabledFalseFlags() {
  return Object.fromEntries(RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_REQUIRED_FALSE_FLAGS.map((key) => [key, false]));
}

function blockedSummary(reason) {
  const fields = {
    schemaId: RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_ID,
    schemaVersion: RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_VERSION,
    contractId: RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_CONTRACT_ID,
    owner: "shell",
    slotOwner: "shell",
    targetKind: "project-envelope-module-downstream-context-summary-slot",
    moduleId: "cs_selector",
    consumerModuleId: "ies_builder",
    state: "ies_first_narrow_candidate_output_artifact_plan_blocked_fail_closed",
    sourceKind: "safe-candidate-output-artifact-plan-summary",
    futureOutputKind: "ies-first-narrow-candidate-output-artifact-plan",
    summaryOnly: true,
    diagnosticOnly: true,
    ...
    readyForArtifactPlanBoundary: false,
    readyForFutureOutput: false,
    artifactPlanJoined: false,
  ...
  delete withoutFingerprint.iesFirstNarrowCandidateOutputArtifactPlanSummaryFingerprint;
  fields.iesFirstNarrowCandidateOutputArtifactPlanSummaryFingerprint = stableFingerprint(
    "safe-ies-first-narrow-candidate-output-artifact-plan-summary",
    withoutFingerprint,
  );
  return Object.freeze(orderedSummary(fields));
}

export function buildRuntimeIesFirstNarrowCandidateOutputArtifactPlanSummary(input = {}) {
  ...
  const unsafe = findUnsafeIesFirstNarrowCandidateOutputArtifactPlanInput({
  ...
  const fields = {
    ...
    readyForArtifactPlanBoundary: true,
    readyForFutureOutput: true,
    artifactPlanJoined: true,
    sourceRowsIncluded: false,
    candidateOutputDetailsIncluded: false,
    manifestListIncluded: false,
    detailListIncluded: false,
    ...
    artifactPlanRecordCount: 1,
    artifactPlanEntryCount: 1,
    firstArtifactPlanEntryKind: "ies_first_narrow_candidate_output_detail_summary_ref",
    ...
    policyFingerprint: safeToken(metadataHandoffSummary.policyFingerprint || candidateOutputSummary.policyFingerprint || manifestSummary.policyFingerprint || detailSummary.policyFingerprint),
    sourceFingerprint: safeToken(metadataHandoffSummary.sourceFingerprint || candidateOutputSummary.sourceFingerprint || manifestSummary.sourceFingerprint || detailSummary.sourceFingerprint),
    sourceInputFingerprint: safeToken(metadataHandoffSummary.sourceInputFingerprint || candidateOutputSummary.sourceInputFingerprint || manifestSummary.sourceInputFingerprint || detailSummary.sourceInputFingerprint),
    boardDataSourceVersion: safeToken(metadataHandoffSummary.boardDataSourceVersion || candidateOutputSummary.boardDataSourceVersion || manifestSummary.boardDataSourceVersion || detailSummary.boardDataSourceVersion),
...
export const buildIesFirstNarrowCandidateOutputArtifactPlanSummary = buildRuntimeIesFirstNarrowCandidateOutputArtifactPlanSummary;
