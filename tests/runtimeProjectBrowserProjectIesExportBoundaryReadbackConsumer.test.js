...
function readyStatus(overrides = {}) {
  return {
    ...
    iesFirstNarrowProjectIesExportBoundaryReadbackFingerprint:
      "safe-ies-first-narrow-project-ies-export-boundary-readback-status:ready-fixture",
    ...overrides,
  };
}
...
function missingStatus(overrides = {}) {
  return {
    ...
    iesFirstNarrowProjectIesExportBoundaryReadbackFingerprint:
      "safe-ies-first-narrow-project-ies-export-boundary-readback-status:missing-fixture",
    ...overrides,
  };
}
...
function blockedStatus(overrides = {}) {
  return {
    ...
    iesFirstNarrowProjectIesExportBoundaryReadbackFingerprint:
      "safe-ies-first-narrow-project-ies-export-boundary-readback-status:blocked-fixture",
    ...overrides,
  };
}
  ...
  return {
    ...
    getStoreSnapshot() {
      return {
        ...
        source: "project-ies-export-boundary-readback-consumer-test-store",
        projects,
    ...
    getProjectEnvelope() {
      throw new Error("project-browser project IES export boundary consumer must not read project envelopes");
    },
    getSelectedResultPersistedSummaryReadbackStatus() {
      throw new Error("project-browser project IES export boundary consumer must not use unrelated readback helpers");
    },
  };
}
...
function assertSummaryShape(summary) {
  ...
  assert.equal(summary.schemaId, PROJECT_BROWSER_PROJECT_IES_EXPORT_BOUNDARY_READBACK_SUMMARY_SCHEMA_ID);
  assert.equal(summary.schemaVersion, PROJECT_BROWSER_PROJECT_IES_EXPORT_BOUNDARY_READBACK_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.owner, "shell");
  assert.equal(summary.source, "project-browser-saved-project-project-ies-export-boundary-readback-consumer");
  ...
  assert.ok(
    summary.projectBrowserProjectIesExportBoundaryReadbackSummaryFingerprint.startsWith(
...
test("project browser consumes only saved-project project IES export boundary readback statuses", () => {
  const projects = [
  ...
  const summary = snapshot.projectIesExportBoundaryReadbackSummary;

  assertSummaryShape(summary);
  assert.equal(summary.state, PROJECT_BROWSER_PROJECT_IES_EXPORT_BOUNDARY_READBACK_STATES.blockedFailClosed);
...
test("project browser blocks malformed readback status without surfacing raw IES or private material", () => {
  const summary = buildProjectBrowserProjectIesExportBoundaryReadbackSummary([
    {
      ...project("unsafe-project", "env-unsafe-project"),
      iesFirstNarrowProjectIesExportBoundaryReadbackStatus: readyStatus({
...
test("project browser fails closed when the safe readback accessor is unavailable or throws", () => {
  const projects = [
  ...
  const summaryWithoutAccessor = buildProjectBrowserProjectIesExportBoundaryReadbackSummary(projects);
  const summaryWithThrowingAccessor = buildProjectBrowserProjectIesExportBoundaryReadbackSummary(
    projects,
    () => {
  ...
  assert.equal(summaryWithoutAccessor.state, PROJECT_BROWSER_PROJECT_IES_EXPORT_BOUNDARY_READBACK_STATES.missing);
  assert.equal(summaryWithoutAccessor.missingProjectCount, 2);
  assert.equal(summaryWithoutAccessor.projectStatuses[0].blocker, "project-ies-export-boundary-readback-status-missing");
  assertNoUnsafeSurface(summaryWithoutAccessor);

  assertSummaryShape(summaryWithThrowingAccessor);
  assert.equal(summaryWithThrowingAccessor.state, PROJECT_BROWSER_PROJECT_IES_EXPORT_BOUNDARY_READBACK_STATES.blockedFailClosed);
  ...
  assert.equal(
    ...
    "project-ies-export-boundary-readback-accessor-failed",
  );
  assertNoUnsafeSurface(summaryWithThrowingAccessor);
});

test("project IES export boundary browser consumer is deterministic, read-only, and adds no route or output behavior", () => {
  const projects = Object.freeze([
    Object.freeze(project("stable-project", "env-stable-project")),
  ]);
  const before = JSON.stringify(projects);
  const first = buildProjectBrowserProjectIesExportBoundaryReadbackSummary(projects, () => readyStatus());
  const second = buildProjectBrowserProjectIesExportBoundaryReadbackSummary(projects, () => readyStatus());

  assertSummaryShape(first);
  assert.equal(first.state, PROJECT_BROWSER_PROJECT_IES_EXPORT_BOUNDARY_READBACK_STATES.ready);
