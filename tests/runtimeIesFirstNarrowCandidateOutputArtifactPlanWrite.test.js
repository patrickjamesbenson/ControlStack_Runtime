...
import {
  buildIesFirstNarrowCandidateOutputArtifactPlanSummary,
...
const POLICY_FINGERPRINT = "safe-policy:ies-first-narrow-candidate-output-artifact-plan-fixture";
const SOURCE_FINGERPRINT = "safe-source:ies-first-narrow-candidate-output-artifact-plan-fixture";
const SOURCE_INPUT_FINGERPRINT = "safe-source-input:ies-first-narrow-candidate-output-artifact-plan-fixture";
const BOARD_DATA_SOURCE_VERSION = "safe-board-data-source-version:ies-first-narrow-candidate-output-artifact-plan-fixture";
...
function context(projectId = "ies-first-narrow-candidate-output-artifact-plan-project") {
  return {
    project: {
      metadata: {
        projectId,
        title: "IES first narrow candidate output artifact-plan project",
    ...
    contractVersion: "ies-first-narrow-candidate-output-artifact-plan-test",
  };
}
...
function sourcePhotometryRefHandoffSummary() {
  return {
    ...
    photometryReferenceFingerprint: "safe-reference:ies-first-narrow-candidate-output-artifact-plan-fixture",
    oneMmPolicyLabel: "one-mm-length-policy-summary-only",
    iesPhotometryReferenceToken: "safe-ies-reference:ies-first-narrow-candidate-output-artifact-plan-fixture",
    lumenCurveReferenceToken: "safe-lumen-curve-reference:ies-first-narrow-candidate-output-artifact-plan-fixture",
    driverUtilCurveReferenceToken: "safe-driver-util-curve-reference:ies-first-narrow-candidate-output-artifact-plan-fixture",
    ...
    rows: [
      {
        rowSchemaId: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_ID,
        rowSchemaVersion: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_VERSION,
        rowKey: "candidate-output-artifact-plan-safe-row-0",
        runKey: "candidate-output-artifact-plan-safe-run-0",
        runIndex: 0,
        rowOrdinal: 1,
...
function assertSummaryShape(summary) {
  assert.deepEqual(Object.keys(summary), RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_FIELD_ORDER);
  assert.equal(summary.schemaId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_ID);
  assert.equal(summary.schemaVersion, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.contractId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_CONTRACT_ID);
  ...
  assert.equal(summary.state, "redacted_ies_first_narrow_candidate_output_artifact_plan_summary_persisted");
  assert.equal(summary.sourceKind, "safe-candidate-output-artifact-plan-summary");
  assert.equal(summary.futureOutputKind, "ies-first-narrow-candidate-output-artifact-plan");
  ...
  assert.equal(summary.readyForArtifactPlanBoundary, true);
  assert.equal(summary.readyForFutureOutput, true);
  assert.equal(summary.artifactPlanJoined, true);
  ...
  assert.equal(summary.firstRowKey, "candidate-output-artifact-plan-safe-row-0");
  assert.equal(summary.firstRowKind, "first_accepted_safe_run_summary");
  assert.equal(summary.firstRunKey, "candidate-output-artifact-plan-safe-run-0");
...
test("builder joins rows, metadata, candidate-output summary, manifest, and detail into an artifact-plan summary", () => {
  const first = buildRuntimeIesFirstNarrowCandidateOutputArtifactPlanSummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
    iesFirstNarrowCandidateOutputSummary: candidateOutputSummary(),
    iesFirstNarrowCandidateOutputManifestSummary: manifestSummary(),
  ...
  const second = buildIesFirstNarrowCandidateOutputArtifactPlanSummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
    iesFirstNarrowCandidateOutputSummary: candidateOutputSummary(),
    iesFirstNarrowCandidateOutputManifestSummary: manifestSummary(),
...
test("writes candidate-output artifact-plan summary only to the shell-owned downstream slot", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(context("ies-candidate-output-artifact-plan-slot-project"), writeContribution());
  ...
  assert.equal(result.iesFirstNarrowCandidateOutputArtifactPlanSummaryTarget, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_TARGET);

  const envelope = result.envelope;
  const downstream = envelope.modules.cs_selector.downstreamContext;
  assert.deepEqual(Object.keys(downstream), ["iesFirstNarrowCandidateOutputArtifactPlanSummary"]);
  assert.equal(envelope.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assert.equal(envelope.project.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assert.equal(envelope.project.metadata.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assert.equal(envelope.project.currentProject.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assert.equal(envelope.project.selection.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assert.equal(envelope.shell.downstream.selector.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assert.equal(envelope.modules.cs_selector.state.iesFirstNarrowCandidateOutputArtifactPlanSummary, undefined);
  assertSummaryShape(downstream.iesFirstNarrowCandidateOutputArtifactPlanSummary);
});

test("can consume the just-written candidate-output detail path in the same envelope save", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-artifact-plan-same-save-project"),
    writeContribution({
      detail: null,
      downstream: {
        iesFirstNarrowCandidateOutputDetailSummaryWrite: { writeRequested: true },
      },
    }),
  );
  ...
  assertSummaryShape(result.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactPlanSummary);
});

test("fails closed when rows, metadata handoff, candidate-output summary, manifest, or detail are missing or not ready", () => {
  const cases = [
  ...
  for (const [label, options, reasonPattern] of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-artifact-plan-${label}-project`),
      writeContribution(options),
    );
    assert.equal(result.accepted, false, label);
    assert.match(result.reason, reasonPattern, label);
    assert.equal(store.getProjectEnvelope(`ies-candidate-output-artifact-plan-${label}-project`), null, label);
  }
});

test("fails closed on raw IES, detail, manifest, artifact plan, photometry, candela, base64, filename, path, and selected-result payload inputs", () => {
  const cases = [
  ...
  for (const [label, payload] of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-artifact-plan-unsafe-${label}`),
      writeContribution({
        downstream: {
          iesFirstNarrowCandidateOutputArtifactPlanSummaryCandidate: payload,
        },
      }),
    );
    assert.equal(result.accepted, false, label);
...
test("fails closed on real generation, donor invocation, artifact-plan file output, routes, POST, and mutation flags", () => {
  const cases = [
  ...
  for (const flag of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-artifact-plan-unsafe-flag-${flag}`),
      writeContribution({
        directWrite: {
          [flag]: true,
        },
...
test("rolls back an existing project update on candidate-output artifact-plan summary write failure", () => {
  const store = createSavedProjectStore();
  const first = store.saveCurrentProjectEnvelope(context("ies-candidate-output-artifact-plan-rollback-project"), writeContribution());
  assert.equal(first.accepted, true, first.reason);
  const before = first.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactPlanSummary;

  const failed = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-artifact-plan-rollback-project"),
    writeContribution({
      downstream: {
        iesFirstNarrowCandidateOutputArtifactPlanSummaryCandidate: {
          rawArtifactPlan: { unsafe: true },
        },
      },
    }),
  );
  ...
  const afterEnvelope = store.getProjectEnvelope("ies-candidate-output-artifact-plan-rollback-project");
  assert.deepEqual(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactPlanSummary, before);
  assert.equal(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactPlanSummary.sourceInputFingerprint, SOURCE_INPUT_FINGERPRINT);
});
