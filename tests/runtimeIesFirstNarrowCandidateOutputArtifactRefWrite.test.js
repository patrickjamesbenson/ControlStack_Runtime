...
import {
  buildIesFirstNarrowCandidateOutputArtifactRefSummary,
...
} from "../packages/workspace-kernel/iesFirstNarrowCandidateOutputArtifactRefSummary.js";
import {
...
} from "../packages/workspace-kernel/iesFirstNarrowCandidateOutputArtifactPlanSummary.js";
import {
...
const POLICY_FINGERPRINT = "safe-policy:ies-first-narrow-candidate-output-artifact-ref-fixture";
const SOURCE_FINGERPRINT = "safe-source:ies-first-narrow-candidate-output-artifact-ref-fixture";
const SOURCE_INPUT_FINGERPRINT = "safe-source-input:ies-first-narrow-candidate-output-artifact-ref-fixture";
const BOARD_DATA_SOURCE_VERSION = "safe-board-data-source-version:ies-first-narrow-candidate-output-artifact-ref-fixture";
const SOURCE_PHOTOMETRY_REF = `safe-source-photometry-ref:${"b".repeat(40)}`;
...
function context(projectId = "ies-first-narrow-candidate-output-artifact-ref-project") {
  return {
    project: {
      metadata: {
        projectId,
        title: "IES first narrow candidate output artifact-ref project",
    ...
    contractVersion: "ies-first-narrow-candidate-output-artifact-ref-test",
  };
}
...
function sourcePhotometryRefHandoffSummary() {
  return {
    ...
    photometryReferenceFingerprint: "safe-reference:ies-first-narrow-candidate-output-artifact-ref-fixture",
    oneMmPolicyLabel: "one-mm-length-policy-summary-only",
    iesPhotometryReferenceToken: "safe-ies-reference:ies-first-narrow-candidate-output-artifact-ref-fixture",
    lumenCurveReferenceToken: "safe-lumen-curve-reference:ies-first-narrow-candidate-output-artifact-ref-fixture",
    driverUtilCurveReferenceToken: "safe-driver-util-curve-reference:ies-first-narrow-candidate-output-artifact-ref-fixture",
    ...
    rows: [
      {
        rowSchemaId: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_ID,
        rowSchemaVersion: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_VERSION,
        rowKey: "candidate-output-artifact-ref-safe-row-0",
        runKey: "candidate-output-artifact-ref-safe-run-0",
        runIndex: 0,
        rowOrdinal: 1,
...
function assertSummaryShape(summary) {
  assert.deepEqual(Object.keys(summary), RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_REF_FIELD_ORDER);
  assert.equal(summary.schemaId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_REF_SUMMARY_SCHEMA_ID);
  assert.equal(summary.schemaVersion, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_REF_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.contractId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_REF_CONTRACT_ID);
  ...
  assert.equal(summary.state, "redacted_ies_first_narrow_candidate_output_artifact_ref_summary_persisted");
  assert.equal(summary.sourceKind, "safe-candidate-output-artifact-ref-summary");
  assert.equal(summary.futureOutputKind, "ies-first-narrow-candidate-output-artifact-ref");
  ...
  assert.equal(summary.firstRowKey, "candidate-output-artifact-ref-safe-row-0");
  assert.equal(summary.firstRowKind, "first_accepted_safe_run_summary");
  assert.equal(summary.firstRunKey, "candidate-output-artifact-ref-safe-run-0");
  ...
  assert.equal(summary.iesFirstNarrowCandidateOutputDetailContractId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_DETAIL_CONTRACT_ID);
  assert.equal(summary.iesFirstNarrowCandidateOutputDetailSchemaId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_DETAIL_SUMMARY_SCHEMA_ID);
  assert.equal(summary.iesFirstNarrowCandidateOutputDetailSchemaVersion, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_DETAIL_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.iesFirstNarrowCandidateOutputDetailState, "redacted_ies_first_narrow_candidate_output_detail_summary_persisted");
  ...
  assert.equal(summary.iesFirstNarrowCandidateOutputArtifactPlanContractId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_CONTRACT_ID);
  assert.equal(summary.iesFirstNarrowCandidateOutputArtifactPlanSchemaId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_ID);
  assert.equal(summary.iesFirstNarrowCandidateOutputArtifactPlanSchemaVersion, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_PLAN_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.iesFirstNarrowCandidateOutputArtifactPlanState, "redacted_ies_first_narrow_candidate_output_artifact_plan_summary_persisted");
...
test("builder joins rows, metadata, candidate-output summary, manifest, detail, and artifact-plan into an artifact-ref summary", () => {
  const first = buildRuntimeIesFirstNarrowCandidateOutputArtifactRefSummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
    iesFirstNarrowCandidateOutputSummary: candidateOutputSummary(),
    iesFirstNarrowCandidateOutputManifestSummary: manifestSummary(),
  ...
  const second = buildIesFirstNarrowCandidateOutputArtifactRefSummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
    iesFirstNarrowCandidateOutputSummary: candidateOutputSummary(),
    iesFirstNarrowCandidateOutputManifestSummary: manifestSummary(),
...
test("writes candidate-output artifact-ref summary only to the shell-owned downstream slot", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(context("ies-candidate-output-artifact-ref-slot-project"), writeContribution());
  ...
  assert.equal(result.iesFirstNarrowCandidateOutputArtifactRefSummaryTarget, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_ARTIFACT_REF_TARGET);

  const envelope = result.envelope;
  const downstream = envelope.modules.cs_selector.downstreamContext;
  assert.deepEqual(Object.keys(downstream), ["iesFirstNarrowCandidateOutputArtifactRefSummary"]);
  assert.equal(envelope.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assert.equal(envelope.project.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assert.equal(envelope.project.metadata.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assert.equal(envelope.project.currentProject.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assert.equal(envelope.project.selection.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assert.equal(envelope.shell.downstream.selector.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assert.equal(envelope.modules.cs_selector.state.iesFirstNarrowCandidateOutputArtifactRefSummary, undefined);
  assertSummaryShape(downstream.iesFirstNarrowCandidateOutputArtifactRefSummary);
});

test("can consume the just-written candidate-output artifact-plan path in the same envelope save", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-artifact-ref-same-save-project"),
    writeContribution({
      artifactPlan: null,
      downstream: {
        iesFirstNarrowCandidateOutputArtifactPlanSummaryWrite: { writeRequested: true },
      },
    }),
  );

  assert.equal(result.accepted, true, result.reason);
  assert.equal(result.iesFirstNarrowCandidateOutputArtifactPlanSummaryWritten, true);
  assert.equal(result.iesFirstNarrowCandidateOutputArtifactRefSummaryWritten, true);
  ...
  assertSummaryShape(result.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactRefSummary);
});

test("fails closed when rows, metadata handoff, candidate-output summary, manifest, detail, or artifact-plan are missing or not ready", () => {
  const cases = [
    ...
    ["missing-detail", { detail: null }, /detail summary|source references|did not join|not ready/],
    ["missing-artifact-plan", { artifactPlan: null }, /artifact-plan summary|source references|did not join|not ready/],
  ];

  for (const [label, options, reasonPattern] of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-artifact-ref-${label}-project`),
      writeContribution(options),
    );
    assert.equal(result.accepted, false, label);
    assert.match(result.reason, reasonPattern, label);
    assert.equal(store.getProjectEnvelope(`ies-candidate-output-artifact-ref-${label}-project`), null, label);
  }
});

test("fails closed on raw IES, detail, manifest, artifact plan, artifact, photometry, candela, base64, filename, path, and selected-result payload inputs", () => {
  const cases = [
    ...
    ["artifactFile", { artifactFile: { output: true } }],
  ...
  for (const [label, payload] of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-artifact-ref-unsafe-${label}`),
      writeContribution({
        downstream: {
          iesFirstNarrowCandidateOutputArtifactRefSummaryCandidate: payload,
        },
      }),
    );
    assert.equal(result.accepted, false, label);
...
test("fails closed on real generation, donor invocation, artifact-ref materialisation, file output, routes, POST, and mutation flags", () => {
  const cases = [
  ...
  for (const flag of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-artifact-ref-unsafe-flag-${flag}`),
      writeContribution({
        directWrite: {
          [flag]: true,
        },
...
test("rolls back an existing project update on candidate-output artifact-ref summary write failure", () => {
  const store = createSavedProjectStore();
  const first = store.saveCurrentProjectEnvelope(context("ies-candidate-output-artifact-ref-rollback-project"), writeContribution());
  assert.equal(first.accepted, true, first.reason);
  const before = first.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactRefSummary;

  const failed = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-artifact-ref-rollback-project"),
    writeContribution({
      downstream: {
        iesFirstNarrowCandidateOutputArtifactRefSummaryCandidate: {
          rawArtifact: { unsafe: true },
        },
      },
    }),
  );
  ...
  const afterEnvelope = store.getProjectEnvelope("ies-candidate-output-artifact-ref-rollback-project");
  assert.deepEqual(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactRefSummary, before);
  assert.equal(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputArtifactRefSummary.sourceInputFingerprint, SOURCE_INPUT_FINGERPRINT);
});
