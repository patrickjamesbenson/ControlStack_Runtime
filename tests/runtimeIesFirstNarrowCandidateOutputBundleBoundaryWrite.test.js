...
import {
  buildIesFirstNarrowCandidateOutputBundleBoundarySummary,
...
import { buildRuntimeIesFirstNarrowCandidateOutputArtifactMaterialisationSummary } from "../packages/workspace-kernel/iesFirstNarrowCandidateOutputArtifactMaterialisationSummary.js";
...
const POLICY_FINGERPRINT = "safe-policy:ies-first-narrow-candidate-output-bundle-boundary-fixture";
const SOURCE_FINGERPRINT = "safe-source:ies-first-narrow-candidate-output-bundle-boundary-fixture";
const SOURCE_INPUT_FINGERPRINT = "safe-source-input:ies-first-narrow-candidate-output-bundle-boundary-fixture";
const BOARD_DATA_SOURCE_VERSION = "safe-board-data-source-version:ies-first-narrow-candidate-output-bundle-boundary-fixture";
...
function context(projectId = "ies-first-narrow-candidate-output-bundle-boundary-project") {
  return {
    project: {
      metadata: {
        projectId,
        title: "IES first narrow candidate output bundle boundary project",
    ...
    contractVersion: "ies-first-narrow-candidate-output-bundle-boundary-test",
  };
}
...
function sourcePhotometryRefHandoffSummary() {
  return {
    ...
    photometryReferenceFingerprint: "safe-reference:ies-first-narrow-candidate-output-bundle-boundary-fixture",
    oneMmPolicyLabel: "one-mm-length-policy-summary-only",
    iesPhotometryReferenceToken: "safe-ies-reference:ies-first-narrow-candidate-output-bundle-boundary-fixture",
    lumenCurveReferenceToken: "safe-lumen-curve-reference:ies-first-narrow-candidate-output-bundle-boundary-fixture",
    driverUtilCurveReferenceToken: "safe-driver-util-curve-reference:ies-first-narrow-candidate-output-bundle-boundary-fixture",
    ...
    rows: [
      {
        rowSchemaId: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_ID,
        rowSchemaVersion: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_VERSION,
        rowKey: "candidate-output-bundle-boundary-safe-row-0",
        runKey: "candidate-output-bundle-boundary-safe-run-0",
        runIndex: 0,
        rowOrdinal: 1,
...
function artifactMaterialisationSummary(overrides = {}) {
  return {
...
function writeContribution(options = {}) {
  const {
    ...
    artifactMaterialisation = artifactMaterialisationSummary(),
  } = options;
  return {
    cs_selector: {
      status: "ready",
      downstreamContext: {
        ...
        iesFirstNarrowCandidateOutputArtifactMaterialisationSummary: artifactMaterialisation,
...
function assertSummaryShape(summary) {
  assert.deepEqual(Object.keys(summary), RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_BUNDLE_BOUNDARY_FIELD_ORDER);
  assert.equal(summary.schemaId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_BUNDLE_BOUNDARY_SUMMARY_SCHEMA_ID);
  assert.equal(summary.schemaVersion, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_BUNDLE_BOUNDARY_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.contractId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_BUNDLE_BOUNDARY_CONTRACT_ID);
  ...
  assert.equal(summary.state, "redacted_ies_first_narrow_candidate_output_bundle_boundary_summary_persisted");
  assert.equal(summary.sourceKind, "safe-candidate-output-bundle-boundary-summary");
  assert.equal(summary.futureOutputKind, "ies-first-narrow-candidate-output-bundle-boundary");
  ...
  assert.match(summary.opaqueBundleBoundaryRef, /^safe-ies-first-narrow-candidate-output-bundle-boundary:[0-9a-f]{40}$/);
  ...
  assert.equal(summary.iesFirstNarrowCandidateOutputArtifactMaterialisationSummaryReady, true);
  assert.equal(summary.readyForArtifactRefBoundary, true);
  assert.equal(summary.readyForMaterialisationBoundary, true);
  assert.equal(summary.readyForBundleBoundary, true);
  ...
  assert.equal(summary.outputBundleBoundaryJoined, true);
  ...
  assert.equal(summary.outputBundleIncluded, false);
  assert.equal(summary.rawOutputBundleIncluded, false);
  ...
  assert.equal(summary.outputBundleRecordCount, 1);
  assert.equal(summary.outputBundleEntryCount, 1);
  assert.equal(summary.firstBundleBoundaryEntryKind, "ies_first_narrow_candidate_output_artifact_materialisation_summary_ref");
  ...
  assert.equal(summary.firstRowKey, "candidate-output-bundle-boundary-safe-row-0");
  assert.equal(summary.firstRowKind, "first_accepted_safe_run_summary");
  assert.equal(summary.firstRunKey, "candidate-output-bundle-boundary-safe-run-0");
  ...
  assert.match(summary.iesFirstNarrowCandidateOutputBundleBoundarySummaryFingerprint, /^safe-ies-first-narrow-candidate-output-bundle-boundary-summary:[0-9a-f]{40}$/);
...
test("builder joins rows, metadata, candidate-output summary, manifest, detail, artifact-plan, artifact-ref, and materialisation into a bundle-boundary summary", () => {
  const first = buildRuntimeIesFirstNarrowCandidateOutputBundleBoundarySummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
    iesFirstNarrowCandidateOutputSummary: candidateOutputSummary(),
    iesFirstNarrowCandidateOutputManifestSummary: manifestSummary(),
  ...
  const second = buildIesFirstNarrowCandidateOutputBundleBoundarySummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
    iesFirstNarrowCandidateOutputSummary: candidateOutputSummary(),
    iesFirstNarrowCandidateOutputManifestSummary: manifestSummary(),
...
test("writes candidate-output bundle-boundary summary only to the shell-owned downstream slot", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(context("ies-candidate-output-bundle-boundary-slot-project"), writeContribution());
  ...
  assert.equal(result.iesFirstNarrowCandidateOutputArtifactMaterialisationSummaryWritten, false);
  assert.equal(result.iesFirstNarrowCandidateOutputBundleBoundarySummaryWritten, true);
  assert.equal(result.iesFirstNarrowCandidateOutputBundleBoundarySummaryTarget, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_BUNDLE_BOUNDARY_TARGET);

  const envelope = result.envelope;
  const downstream = envelope.modules.cs_selector.downstreamContext;
  assert.deepEqual(Object.keys(downstream), ["iesFirstNarrowCandidateOutputBundleBoundarySummary"]);
  assert.equal(envelope.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assert.equal(envelope.project.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assert.equal(envelope.project.metadata.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assert.equal(envelope.project.currentProject.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assert.equal(envelope.project.selection.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assert.equal(envelope.shell.downstream.selector.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assert.equal(envelope.modules.cs_selector.state.iesFirstNarrowCandidateOutputBundleBoundarySummary, undefined);
  assertSummaryShape(downstream.iesFirstNarrowCandidateOutputBundleBoundarySummary);
});

test("can consume the just-written candidate-output artifact-materialisation path in the same envelope save", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-bundle-boundary-same-save-project"),
    writeContribution({
      artifactMaterialisation: null,
      downstream: {
        iesFirstNarrowCandidateOutputArtifactMaterialisationSummaryWrite: { writeRequested: true },
      },
    }),
  );
  ...
  assertSummaryShape(result.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputBundleBoundarySummary);
});

test("fails closed when rows, metadata handoff, candidate-output summary, manifest, detail, artifact-plan, artifact-ref, or materialisation are missing or not ready", () => {
  const cases = [
    ...
    ["missing-artifact-ref", { artifactRef: null }, /artifact-ref|dependencies|not ready|did not join|source references/],
    ["missing-artifact-materialisation", { artifactMaterialisation: null }, /artifact-materialisation summary|not ready|did not join|source references/],
  ];

  for (const [label, options, reasonPattern] of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-bundle-boundary-${label}-project`),
      writeContribution(options),
    );
    assert.equal(result.accepted, false, label);
    assert.match(result.reason, reasonPattern, label);
    assert.equal(store.getProjectEnvelope(`ies-candidate-output-bundle-boundary-${label}-project`), null, label);
  }
});

test("fails closed on raw IES, materialisation, bundle, photometry, candela, base64, filename, path, and selected-result payload inputs", () => {
  const cases = [
    ...
    ["outputBundle", { outputBundle: { unsafe: true } }],
    ["rawOutputBundle", { rawOutputBundle: { unsafe: true } }],
    ["rawOutputBundlePayload", { rawOutputBundlePayload: { unsafe: true } }],
    ["bundleFiles", { bundleFiles: ["candidate.ies"] }],
  ...
  for (const [label, payload] of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-bundle-boundary-unsafe-${label}`),
      writeContribution({
        downstream: {
          iesFirstNarrowCandidateOutputBundleBoundarySummaryCandidate: payload,
        },
      }),
    );
    assert.equal(result.accepted, false, label);
...
test("fails closed on real generation, donor invocation, materialisation, bundle output, file output, routes, POST, and mutation flags", () => {
  const cases = [
  ...
  for (const flag of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-bundle-boundary-unsafe-flag-${flag}`),
      writeContribution({
        directWrite: {
          [flag]: true,
        },
...
test("rolls back an existing project update on candidate-output bundle-boundary summary write failure", () => {
  const store = createSavedProjectStore();
  const first = store.saveCurrentProjectEnvelope(context("ies-candidate-output-bundle-boundary-rollback-project"), writeContribution());
  assert.equal(first.accepted, true, first.reason);
  const before = first.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputBundleBoundarySummary;

  const failed = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-bundle-boundary-rollback-project"),
    writeContribution({
      downstream: {
        iesFirstNarrowCandidateOutputBundleBoundarySummaryCandidate: {
          rawOutputBundle: { unsafe: true },
        },
      },
    }),
  );
  ...
  const afterEnvelope = store.getProjectEnvelope("ies-candidate-output-bundle-boundary-rollback-project");
  assert.deepEqual(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputBundleBoundarySummary, before);
  assert.equal(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputBundleBoundarySummary.sourceInputFingerprint, SOURCE_INPUT_FINGERPRINT);
});
