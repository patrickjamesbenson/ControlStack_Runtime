...
test("Selector selected-truth summary renders before workflow controls and Diagnostics", async () => {
  ...
  const summaryRenderIndex = source.indexOf("appendSelectorSelectionTruthSummary(section");
  ...
  assert.ok(summaryRenderIndex > 0, "selected-truth summary should be rendered");
  assert.ok(workflowIndex > summaryRenderIndex, "workflow controls should render after selected-truth summary");
  assert.ok(diagnosticsIndex > workflowIndex, "Diagnostics should remain after the product workflow");
