...
function project(projectId, envelopeId, status = undefined) {
  return {
    ...
    ...(status ? { iesFirstNarrowProjectIesExportResultReadbackStatus: status } : {}),
  };
}
...
function readyStatus(overrides = {}) {
  return {
    ...
    iesFirstNarrowProjectIesExportResultReadbackFingerprint:
      "safe-ies-first-narrow-project-ies-export-result-readback-status:selected-detail-ready-fixture",
    ...overrides,
  };
}
...
function missingStatus(overrides = {}) {
  return {
    ...
    iesFirstNarrowProjectIesExportResultReadbackFingerprint:
      "safe-ies-first-narrow-project-ies-export-result-readback-status:selected-detail-missing-fixture",
    ...overrides,
  };
}
...
function blockedStatus(overrides = {}) {
  return {
    ...
    iesFirstNarrowProjectIesExportResultReadbackFingerprint:
      "safe-ies-first-narrow-project-ies-export-result-readback-status:selected-detail-blocked-fixture",
    ...overrides,
  };
}
...
function assertDetailShape(detail) {
  ...
  assert.equal(detail.schemaId, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_SUMMARY_SCHEMA_ID);
  assert.equal(detail.schemaVersion, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_SUMMARY_SCHEMA_VERSION);
  assert.equal(detail.owner, "shell");
  assert.equal(detail.source, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_SOURCE);
  assert.equal(detail.sourceSummarySchemaId, PROJECT_BROWSER_PROJECT_IES_EXPORT_RESULT_READBACK_SUMMARY_SCHEMA_ID);
  assert.equal(detail.sourceSummarySchemaVersion, PROJECT_BROWSER_PROJECT_IES_EXPORT_RESULT_READBACK_SUMMARY_SCHEMA_VERSION);
  ...
  assert.ok(
    detail.sourceProjectBrowserProjectIesExportResultReadbackSummaryFingerprint.startsWith(
      "safe-project-browser-project-ies-export-result-readback-summary:",
    ),
  );
  assert.ok(
    detail.projectBrowserSelectedProjectIesExportResultReadbackDetailFingerprint.startsWith(
...
test("selected-project IES export result detail consumes the redacted aggregate summary only", () => {
  const aggregate = buildProjectBrowserProjectIesExportResultReadbackSummary([
  ...
  assert.equal(detail.sourceSummaryState, PROJECT_BROWSER_PROJECT_IES_EXPORT_RESULT_READBACK_STATES.blockedFailClosed);
  assert.equal(detail.sourceSummaryReadiness, "blocked_fail_closed");
  assert.equal(detail.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.ready);
  ...
  assert.equal(detail.resultReadbackState, PROJECT_BROWSER_PROJECT_IES_EXPORT_RESULT_READBACK_STATES.ready);
  assert.equal(detail.resultReadbackReadiness, "ready");
  assert.equal(detail.resultReadbackReady, true);
  assert.equal(detail.resultReadbackFailClosed, false);
  assert.equal(detail.resultReadbackBlocker, null);
  assert.equal(
...
test("selected-project IES export result detail distinguishes missing selection, invalid id, and not found", () => {
  const aggregate = buildProjectBrowserProjectIesExportResultReadbackSummary([
    project("ready-project", "env-ready-project", readyStatus()),
  ]);

  const notSelected = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(aggregate);
  const invalid = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(aggregate, "C:\\private\\project");
  const notFound = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(
    aggregate,
    "env-missing-project",
  );

  assertDetailShape(notSelected);
  assert.equal(notSelected.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.missing);
  ...
  assertDetailShape(invalid);
  assert.equal(invalid.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.blockedFailClosed);
  ...
  assertNoUnsafeSurface(invalid);

  assertDetailShape(notFound);
  assert.equal(notFound.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.missing);
...
test("selected-project IES export result detail preserves missing and blocked fail-closed diagnostics", () => {
  const aggregate = buildProjectBrowserProjectIesExportResultReadbackSummary([
    project("missing-project", "env-missing-project", missingStatus()),
    project("blocked-project", "env-blocked-project", blockedStatus()),
  ]);

  const missing = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(
    aggregate,
    "env-missing-project",
  );
  const blocked = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(
    aggregate,
    "env-blocked-project",
  );

  assertDetailShape(missing);
  assert.equal(missing.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.missing);
  assert.equal(missing.readiness, "missing");
  assert.equal(missing.ready, false);
  assert.equal(missing.failClosed, true);
  assert.equal(missing.blocker, "project-ies-export-result-summary-slot-empty");
  assert.equal(missing.resultReadbackReadiness, "missing");
  assert.equal(missing.resultReadbackFailClosed, true);
  assertNoUnsafeSurface(missing);

  assertDetailShape(blocked);
  assert.equal(blocked.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.blockedFailClosed);
  ...
  assert.equal(blocked.resultReadbackReadiness, "blocked_fail_closed");
  assert.equal(blocked.resultReadbackFailClosed, true);
  assertNoUnsafeSurface(blocked);
});

test("selected-project IES export result detail blocks malformed source status without projecting raw material", () => {
  const unsafeAggregate = {
    ...
    projectBrowserProjectIesExportResultReadbackSummaryFingerprint:
      "safe-project-browser-project-ies-export-result-readback-summary:unsafe-fixture",
  };
  ...
  assertDetailShape(detail);
  assert.equal(detail.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.blockedFailClosed);
...
test("project-browser snapshot derives result detail from one safe readback pass and never opens envelopes", () => {
  ...
  const store = {
    getStoreSnapshot() {
      return {
        ...
        source: "selected-project-ies-export-result-detail-test-store",
        projects,
    ...
    getProjectEnvelope() {
      throw new Error("selected-project IES export result detail consumer must not open envelopes");
    },
    saveCurrentProjectEnvelope() {
      throw new Error("selected-project IES export result detail consumer must not mutate project state");
    },
    restoreProjectEnvelope() {
      throw new Error("selected-project IES export result detail consumer must not restore project state");
    },
    prepareHandoffSharePackage() {
      throw new Error("selected-project IES export result detail consumer must not prepare output paths");
    },
  };
  const service = createProjectBrowserService({
    ...
    projectService: {
      restoreProjectFromEnvelope() {
        throw new Error("selected-project IES export result detail consumer must not invoke restore paths");
      },
      recordHandoffSharePackage() {
        throw new Error("selected-project IES export result detail consumer must not invoke output paths");
      },
    },
    eventBus: {
      emit() {
  ...
  const detail = snapshot.selectedProjectIesExportResultReadbackDetailSummary;

  assertDetailShape(detail);
  assert.equal(detail.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.missing);
  assert.equal(detail.blocker, "project-browser-selected-project-not-selected");
  assert.equal(
    detail.sourceProjectBrowserProjectIesExportResultReadbackSummaryFingerprint,
    snapshot.projectIesExportResultReadbackSummary.projectBrowserProjectIesExportResultReadbackSummaryFingerprint,
  );
  assert.deepEqual(calls, ["env-ready-project", "env-missing-project"]);
  assertNoUnsafeSurface(detail);
});

test("selected-project IES export result detail is deterministic and does not mutate source summaries", () => {
  const aggregate = Object.freeze(buildProjectBrowserProjectIesExportResultReadbackSummary([
    Object.freeze(project("stable-project", "env-stable-project", readyStatus())),
  ]));
  const before = JSON.stringify(aggregate);

  const first = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(
    aggregate,
    "env-stable-project",
  );
  const second = buildProjectBrowserSelectedProjectIesExportResultReadbackDetailSummary(
    aggregate,
    "env-stable-project",
  );

  assertDetailShape(first);
  assert.equal(first.state, PROJECT_BROWSER_SELECTED_PROJECT_IES_EXPORT_RESULT_READBACK_DETAIL_STATES.ready);
