...
import {
  buildIesFirstNarrowCandidateOutputSummary,
...
const POLICY_FINGERPRINT = "safe-policy:ies-first-narrow-candidate-output-fixture";
const SOURCE_FINGERPRINT = "safe-source:ies-first-narrow-candidate-output-fixture";
const SOURCE_INPUT_FINGERPRINT = "safe-source-input:ies-first-narrow-candidate-output-fixture";
const BOARD_DATA_SOURCE_VERSION = "safe-board-data-source-version:ies-first-narrow-candidate-output-fixture";
...
function context(projectId = "ies-first-narrow-candidate-output-project") {
  return {
    project: {
      metadata: {
        projectId,
        title: "IES first narrow candidate output project",
    ...
    contractVersion: "ies-first-narrow-candidate-output-test",
  };
}
...
function sourcePhotometryRefHandoffSummary() {
  return {
    ...
    photometryReferenceFingerprint: "safe-reference:ies-first-narrow-candidate-output-fixture",
    oneMmPolicyLabel: "one-mm-length-policy-summary-only",
    iesPhotometryReferenceToken: "safe-ies-reference:ies-first-narrow-candidate-output-fixture",
    lumenCurveReferenceToken: "safe-lumen-curve-reference:ies-first-narrow-candidate-output-fixture",
    driverUtilCurveReferenceToken: "safe-driver-util-curve-reference:ies-first-narrow-candidate-output-fixture",
    ...
    rows: [
      {
        rowSchemaId: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_ID,
        rowSchemaVersion: RUNTABLE_FIRST_NARROW_ROW_SCHEMA_VERSION,
        rowKey: "candidate-output-safe-row-0",
        runKey: "candidate-output-safe-run-0",
        runIndex: 0,
        rowOrdinal: 1,
...
function assertSummaryShape(summary) {
  assert.deepEqual(Object.keys(summary), RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_FIELD_ORDER);
  assert.equal(summary.schemaId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_SUMMARY_SCHEMA_ID);
  assert.equal(summary.schemaVersion, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_SUMMARY_SCHEMA_VERSION);
  assert.equal(summary.contractId, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_CONTRACT_ID);
  ...
  assert.equal(summary.state, "redacted_ies_first_narrow_candidate_output_summary_persisted");
  assert.equal(summary.sourceKind, "safe-runtable-rows-and-ies-metadata-summary");
  assert.equal(summary.futureOutputKind, "ies-first-narrow-candidate-output");
  ...
  assert.equal(summary.readyForSafeCandidateBoundary, true);
  assert.equal(summary.readyForFutureOutput, true);
  assert.equal(summary.rowsJoined, true);
  ...
  assert.equal(summary.firstRowKey, "candidate-output-safe-row-0");
  assert.equal(summary.firstRowKind, "first_accepted_safe_run_summary");
  assert.equal(summary.firstRunKey, "candidate-output-safe-run-0");
...
test("builder joins RunTable first narrow rows and IES first narrow metadata into candidate-output summary", () => {
  const first = buildRuntimeIesFirstNarrowCandidateOutputSummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
  });
  const second = buildIesFirstNarrowCandidateOutputSummary({
    runTableFirstNarrowRows: runTableFirstNarrowRows(),
    iesFirstNarrowMetadataHandoffSummary: metadataHandoffSummary(),
  });
...
test("writes candidate-output summary only to the shell-owned downstream slot", () => {
  const store = createSavedProjectStore();
  const result = store.saveCurrentProjectEnvelope(context("ies-candidate-output-slot-project"), writeContribution());
  ...
  assert.equal(result.iesFirstNarrowCandidateOutputSummaryTarget, RUNTIME_IES_FIRST_NARROW_CANDIDATE_OUTPUT_TARGET);

  const envelope = result.envelope;
  const downstream = envelope.modules.cs_selector.downstreamContext;
  assert.deepEqual(Object.keys(downstream), ["iesFirstNarrowCandidateOutputSummary"]);
  assert.equal(envelope.iesFirstNarrowCandidateOutputSummary, undefined);
  assert.equal(envelope.project.iesFirstNarrowCandidateOutputSummary, undefined);
  assert.equal(envelope.project.metadata.iesFirstNarrowCandidateOutputSummary, undefined);
  assert.equal(envelope.project.currentProject.iesFirstNarrowCandidateOutputSummary, undefined);
  assert.equal(envelope.project.selection.iesFirstNarrowCandidateOutputSummary, undefined);
  assert.equal(envelope.shell.downstream.selector.iesFirstNarrowCandidateOutputSummary, undefined);
  assert.equal(envelope.modules.cs_selector.state.iesFirstNarrowCandidateOutputSummary, undefined);
  assertSummaryShape(downstream.iesFirstNarrowCandidateOutputSummary);
});

test("fails closed when RunTable rows or metadata handoff are missing or not ready", () => {
  const missingRowsStore = createSavedProjectStore();
  const missingRows = missingRowsStore.saveCurrentProjectEnvelope(
    context("ies-candidate-output-missing-rows-project"),
    writeContribution({ rows: null }),
  );
  assert.equal(missingRows.accepted, false);
  assert.match(missingRows.reason, /source references|did not join|row inclusion|not ready/);
  assert.equal(missingRowsStore.getProjectEnvelope("ies-candidate-output-missing-rows-project"), null);
  ...
  const notReadyMetadata = notReadyMetadataStore.saveCurrentProjectEnvelope(
    context("ies-candidate-output-not-ready-metadata-project"),
  ...
  assert.equal(notReadyMetadataStore.getProjectEnvelope("ies-candidate-output-not-ready-metadata-project"), null);
});

test("fails closed on raw IES, photometry, candela, base64, filename, path, and selected-result payload inputs without re-exposing raw key names", () => {
  const cases = [
  ...
  for (const [label, payload] of cases) {
    ...
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-unsafe-${label}`),
      writeContribution({
        downstream: {
          iesFirstNarrowCandidateOutputSummaryCandidate: payload,
        },
      }),
    );
...
test("fails closed on real generation, donor invocation, file output, routes, POST, and mutation flags", () => {
  const cases = [
  ...
  for (const flag of cases) {
    const store = createSavedProjectStore();
    const result = store.saveCurrentProjectEnvelope(
      context(`ies-candidate-output-unsafe-flag-${flag}`),
      writeContribution({
        directWrite: {
          [flag]: true,
        },
...
test("rolls back an existing project update on candidate-output summary write failure", () => {
  const store = createSavedProjectStore();
  const first = store.saveCurrentProjectEnvelope(context("ies-candidate-output-rollback-project"), writeContribution());
  assert.equal(first.accepted, true, first.reason);
  const before = first.envelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputSummary;

  const failed = store.saveCurrentProjectEnvelope(
    context("ies-candidate-output-rollback-project"),
    writeContribution({
      downstream: {
  ...
  const afterEnvelope = store.getProjectEnvelope("ies-candidate-output-rollback-project");
  assert.deepEqual(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputSummary, before);
  assert.equal(afterEnvelope.modules.cs_selector.downstreamContext.iesFirstNarrowCandidateOutputSummary.sourceInputFingerprint, SOURCE_INPUT_FINGERPRINT);
});
