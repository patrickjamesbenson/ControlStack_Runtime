...
**Batch rule:** one Selector writer and one parcel at a time. WALK-001 is accepted complete. WALK-002 is the sole commissioned parcel under Program's exact bounded production and focused-test scope. Stop for Program review after WALK-002; do not continue into WALK-003. No live Engine run is authorised until the later walkthrough cleanup is accepted. PWS-001, PWS-002 and PWS-005 are already done and must not be recommissioned.

### WALK-001 Refresh active authority snapshot
* status: done (corrected source passed every guard; authority materialised, prior authority archived before promotion, promoted authority independently verified; Program accepted)
...
* status: blocked (read-only trace found required production owners outside Program-authorised scope; no implementation change)
* objective: first trace every current `runLengthMode` owner and consumer read-only. Then delete Length Mode as Selector authority from run-intake state and validation, committed or persisted Selector constraints, Build Ready and run-readiness requirements and diagnostics, factory-approved summaries, spine and payload previews, Selector reference-option metadata, and rendered controls, labels and next-action wording.
* ruling: preserve `runQty` and positive physical `runLength` as the only run-intake decisions. A run with both must not be incomplete because no Length Mode exists. Do not create or infer a replacement, default, fallback, sentinel, derived value or hidden constraint. Old stored values are ignored and omitted from new payloads, summaries and UI.
* candidate ruling: preserve all otherwise eligible source-backed NVB Board candidates for downstream Lex scoring. Removal must preserve or broaden candidate availability, never narrow it or preselect a manufacturing interpretation.
* authorised production paths: `packages/modules/cs-selector/selectorRunIntakePreview.js`; `packages/modules/cs-selector/selectorFactoryApprovedInputsSummary.js`; `packages/modules/cs-selector/selectorView.js`; `packages/modules/cs-selector/selectorViewModel.js`; `packages/workspace-kernel/selectorReferenceOptionsService.js`.
* focused test paths only: `tests/selectorRunIntakePreview.test.js`; `tests/selectorRunIntentCaptureSurface.test.js`; `tests/selectorSpecBuildReadinessPreview.test.js`; `tests/selectorSpinePayloadSkeleton.test.js`; `tests/selectorSafeWorkflowSurfaceIntegration.test.js`; `tests/goldenSupportedMachineValueWitness.test.js`.
* acceptance: no Length Mode control or option metadata remains; readiness needs quantity and physical length only; no hidden replacement/default exists; stored Length Mode is ignored and omitted; payloads, summaries and spine rows contain no Length Mode; quantity and physical length remain functional; eligible NVB candidate breadth is unchanged or broader; safe-local-intent and all no-write/disabled-production boundaries remain intact.
* stop: if read-only trace finds a required production owner outside the authorised production list, stop with the exact diagnosis and do not broaden scope.
* boundary: no Engine or RunTable invocation, IES generation, persistence, new route/POST action, RuntimeData mutation, saved-seed execution or mutation, CRM/provider action, artifact retrieval/delivery, cross-lane/main/donor write, duplicate-scaffold work or later walkthrough parcel.
* receipt: run all affected focused tests and the fixed Selector gate; commit and push the exact parcel; reconcile lane memory; stop for Program review without beginning WALK-003.
* read-only trace stop: required out-of-scope owners are `apps/workspace-shell/src/projectBrowserSelectedProjectServerOwnedRegistrationClientTransport.js` (registration readiness still requires non-empty mode), `packages/workspace-kernel/projectEnvelope.js` (persisted envelope allowlist and exact-shape validation require mode), `packages/workspace-kernel/selectorReadonlyEngineCandidateMapper.js` (fingerprint, `cut_to_length` fallback and new payload fields), `packages/modules/cs-selector/selectorRunAccessoryPlacementPreview.js` (hidden same-length manufacturing interpretation), `packages/workspace-kernel/engineRunTableSealedCandidateAssemblyPreview.js` and `packages/workspace-kernel/engineRunTableRuntimeRunTableDomainOutputScaffold.js` (`unresolved` fallback plus downstream mode summaries). Program rescope is required; no production/test change, gate, stage, commit or runtime action occurred.
...
* authorised documentation: canonical Selector lane-memory files for commissioning and closeout only.
* safe summary: SYSTEM_POLICY table presence and row count; exact `ambient_temp` row count; approved exact `ambient_temp` row count; presence and population counts for recognised `economy`, `business`, `first` and `charter` tier columns; finite and non-finite numeric-token counts by recognised tier column; rows with at least one finite tier value; rows with finite values across multiple recognised tier columns; generic-only rows that do not prove tier gating.
* disclosure boundary: return no policy values, raw rows, unrestricted headers, users, credentials, Google Sheet identity, provider content, complete source JSON or private locations.
* gate-included acceptance: prove finite tier-specific Ambient evidence; generic-only values do not prove Tier gating; non-numeric values do not prove finite Ambient; duplicate exact Ambient rows remain visible through safe counts; disclosure remains redacted; dry-run writes nothing.
* unchanged boundary: Driver authority rules, current source-validation outcomes, Google reading, Google data, Selector option behaviour, materialisation, archive, promotion, deployment and Engine.
* closeout: gate and push the exact bounded parcel, update lane memory, and stop for Program activation and exact source-repair review.
