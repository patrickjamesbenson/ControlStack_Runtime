...
- Tier summary classification: Tier still appears as `missing` only in broader donor-parity presentation. The dedicated candidate mapper is ready without it, client Tier is non-authoritative, and server-owned derivation remains at `SYSTEM_POLICY` — NOT THE CURRENT REGISTRATION BLOCKER.
- Spec/build classification: the readiness card reports Ambient missing and Mounting/Finishes incomplete, with no incompatible selections. Ambient is current-source unavailable; Mounting/Finishes are user-selectable broader build inputs. None is required by the approved first readonly registration candidate — VERIFIED FROM USER-PROVIDED BROWSER EVIDENCE AND REPOSITORY CONTRACT.
- Broad totals classification: the 13 missing and 18 blocked totals include future-mapped fields, consequence-only values, unavailable source metadata, diagnostic rows, and intentionally disabled downstream outputs; they are not the registration prerequisite list — VERIFIED FROM THE COMPLETE BROWSER DUMP.
