...
PWS-001, PWS-002 and PWS-005 are complete. C5 and the Lab seed-memory correction are accepted complete; no Lab parcel is ready. WALK-001, WALK-001V and WALK-001VA are accepted complete. WALK-002 Length Mode deletion is the sole ready Selector parcel. One Selector writer only.
...
- **WALK-002 — Length Mode removal:** ready; sole Selector parcel. Diagnose current downstream effect read-only, then delete `runLengthMode` from run intake, committed/persisted constraints, readiness, summaries, spine/payload preview, option metadata and rendered UI. Preserve quantity and physical length, create no replacement/default/sentinel, and keep all otherwise eligible NVB candidates available for downstream Lex scoring.
