...
- **SS-1G — Governance Sales Signals guard activation:** ready; sole Governance infrastructure prerequisite. Add exactly `packages/modules/sales-signals/**`, `tests/salesSignals*.test.js`, and `docs/sales-signals/**` to the managed Governance MCP guard. Preserve all existing globs, prohibit broad module/test wildcards, reload the resident manager, recycle the complete Governance MCP host, start fresh through the manager, run no-change scope probes, and leave unrelated services untouched.
- **SS-1 — schema and classification harness:** blocked behind SS-1G acceptance. Deliver only the opportunity-signal schema contract document and deterministic keyword-classification harness/correction corpus against manually supplied alerts. No mailbox automation, EstimateOne API, live HubSpot writes, Selector surface, shell registration, persistence, retrieval or delivery.
