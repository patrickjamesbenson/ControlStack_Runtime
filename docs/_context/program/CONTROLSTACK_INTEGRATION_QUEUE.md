...
- **WALK-002R — complete Length Mode owner deletion:** ready; sole Selector parcel. Remove every current `runLengthMode`/`lengthMode` production occurrence across the exact eleven-file scope recorded in the Program handoff. Preserve quantity and physical length only, discard legacy mode data, remove all defaults/sentinels/proxies, keep Board candidate breadth equal or broader for Lex scoring, and preserve every no-Engine/no-generation/no-persistence boundary.
