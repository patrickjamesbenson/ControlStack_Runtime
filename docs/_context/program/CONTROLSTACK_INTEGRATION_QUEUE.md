...
- **WALK-003 — duplicate scaffold removal:** ready; sole Selector parcel. Exact production boundary: `selectorState.js`, `selectorViewModel.js` and `selectorView.js`. Unmount the runtime-native single-page expander scaffold, remove its fabricated module-local default current-selection ownership, preserve the genuine source-backed current-selection rail, couple-delete or rewrite every test encoding the old scaffold/default authority with counted residual zero, gate, push and stop before WALK-004.
