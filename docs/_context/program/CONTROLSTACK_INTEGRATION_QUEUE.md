...
- **WALK-001 — authority refresh:** open; not complete. Selector reports a correct fail-closed stop before all writes because the running service could not load the server-side Google Sheets reader. No Ambient/source validation, materialisation, archive or promotion is accepted yet.
- **WALK-001R — Google reader recovery and WALK-001 resume:** blocked on WALK-001I. Selector reproduced the safe reader-load failure and reported the serving worktree's declared locked dependency set is not installed. No credential or code fault was identified; no write occurred.
- **WALK-001I — locked service dependency installation:** approved Program operational parcel; Patrick executes under advisor guidance. Run `npm ci` only in the exact Selector worktree serving port 8788, then use the existing ControlStack Manager panel to restart only `selector-runtime` / `CS Selector Runtime / Workspace Shell (8788)`. The package manifest and lockfile must remain unchanged; do not use `npm install`, change credentials, modify deployment configuration or restart another service. After Healthy/Managed recovery and clean Selector Git verification, WALK-001R resumes the complete original dry-run. Only after the dry-run and finite tier-gated `SYSTEM_POLICY.ambient_temp` proof pass may the existing materialise/archive/promote workflow continue.
...
- **Order:** WALK-001 → WALK-002 → WALK-003 → WALK-004 → WALK-005 → WALK-005A → WALK-006 → WALK-007. WALK-008 is ruled and closed.
