...
- **WALK-001R — Google reader recovery and WALK-001 resume:** ready; sole Selector parcel. Reproduce the safe reader-load failure, diagnose only the running Selector service's dependency/runtime/configuration boundary, and repair it only through bounded Selector-owned or existing managed-service controls. Do not expose or relocate credentials. If recovery requires Program-owned deployment changes, a new secret binding, cross-lane writes or broad environment work, stop with a redacted diagnosis. Restart only `selector-runtime` through the existing manager when required, then rerun the original guarded dry-run. Only after the complete dry-run and finite tier-gated `SYSTEM_POLICY.ambient_temp` proof pass may the already-authorised materialise/archive/promote workflow continue.
