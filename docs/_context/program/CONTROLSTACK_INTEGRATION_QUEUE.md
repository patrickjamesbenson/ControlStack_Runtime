...
PWS-001, PWS-002 and PWS-005 are complete. C5 and the Lab seed-memory correction are accepted complete; no Lab parcel is ready. Governance GOV-002 remains done and no Governance parcel is ready. WALK-001 through WALK-003, including WALK-002R and both guard activations, are accepted complete. WALK-004 is the sole ready Selector parcel under one writer.
...
- **WALK-004 — legacy TIERS option/readiness deletion:** ready after accepted protected scope stop; sole Selector parcel under the existing worker. Exact production boundary: `packages/workspace-kernel/selectorReferenceOptionsService.js` and `packages/modules/cs-selector/selectorFactoryApprovedInputsSummary.js` only. Remove manual Tier/`TIERS` option authority, committed Tier candidate readiness and committed-Tier Stage 3B authority. Stage 3B may consume only the valid server-derived `SYSTEM_POLICY` policy-summary tier. Preserve the 15-table source-status bridge, server/Engine Tier derivation, legitimate downstream Tier/profile result fields and genuinely non-TIERS Electrical Class. Recount coupled tests from the restored baseline, finish at residual zero, gate, push and stop before WALK-005.
...
- **SS-1 — schema and classification harness:** ready; sole Governance parcel under one writer. Deliver only the opportunity-signal schema contract document and deterministic keyword-classification harness/correction corpus against manually supplied redacted alerts. Require stable output order, explicit reason codes, false-positive and false-negative correction cases, and fail-closed rejection of malformed, path/URL/provider/secret-bearing input. No mailbox automation, EstimateOne API, live HubSpot reads or writes, Selector surface, shell registration, persistence, retrieval or delivery.
