...
- **WALK-004 — legacy TIERS option/readiness deletion:** ready; sole Selector parcel. Exact production boundary: `packages/workspace-kernel/selectorReferenceOptionsService.js` only. Remove the manual Tier workflow field, `TIERS` alias/debug exposure, all `TIERS` reads, Tier option emission, TIERS-parented electrical-class options and TIERS relationship records. Preserve the current 15-table source-status bridge, server/Engine `SYSTEM_POLICY` Tier derivation and any genuinely non-TIERS electrical-class source. Couple-delete or rewrite every test encoding the old authority with counted residual zero, gate, push and stop before WALK-005.
