...
- **WALK-002RG — exact Selector write-guard activation:** ready; sole infrastructure prerequisite. Program's published deployment configuration adds exactly the run-intake preview and accessory-placement preview files to the Selector MCP guard, with no wildcard broadening. Advisor applies the configuration, restarts only Selector MCP through the existing manager and verifies the live exact permission set. WALK-002R then resumes with the same worker and parcel.
