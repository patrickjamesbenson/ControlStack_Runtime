...
- **WALK-002R — complete Length Mode owner deletion:** blocked pending WALK-002RGT exact golden-witness test activation. The twelve-owner scope, four banned-name zero searches, equal-or-broader Board candidate rule and deletion-test coupling remain unchanged. Baseline remains 71 encoded assumptions across 32 test files; final residual must be zero.
...
- **WALK-002RGT — exact golden-witness test guard activation:** ready; sole infrastructure prerequisite. Add only the authorised golden supported-machine witness test to the live guard, retain all nine exact module permissions and existing test patterns, keep broad module and broad test wildcards absent, reload the resident manager configuration, recycle the complete Selector MCP host, and prove the test path passes a no-change scope check. The existing WALK-002R worker then resumes.
