...
WALK-003 is the sole ready Selector parcel under one writer. Exact production scope is `selectorState.js`, `selectorViewModel.js` and `selectorView.js`. Remove the mounted single-page expander scaffold and its fabricated module-local default current-selection ownership, including static `Linear 60`, `1200 lm/m`, `IK07` and other default-preview values as effective selection or derived authority. Preserve the source-backed live surface and genuine current-selection rail; a fresh load may remain empty/incomplete but must carry no selected product truth.

Deletion-test coupling applies with a mechanical pre-edit baseline and final residual zero. Do not remove genuine source-backed fixtures containing the same real values, do not begin donor auto-fill/default acknowledgement early, and stop if a fourth production owner is required. Gate, push the exact parcel and lane-memory closeout, leave the lane clean and stop before WALK-004.
