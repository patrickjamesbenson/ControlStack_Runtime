...
PWS-001, PWS-002 and PWS-005 are complete and are not reopened. WALK-001 remains open after a correctly fail-closed stop before all writes: Selector reports that the running service could not load the server-side Google Sheets reader. No Ambient/source validation, materialisation, archive or promotion is accepted. WALK-001R is blocked on the approved Program operational action WALK-001I.

WALK-001R reproduced the redacted failure and stopped again. Selector reports that the package manifest and lockfile already declare and pin `googleapis`, but the serving worktree's installed dependencies are incomplete. Credentials and tracked files were untouched and the lane is clean.

WALK-001I is the sole ready recovery action. Patrick runs `npm ci` in the exact Selector worktree serving port 8788, then uses the existing ControlStack Manager panel to restart only `CS Selector Runtime / Workspace Shell (8788)`. Package files, credentials, deployment configuration and every other service remain unchanged. After Healthy/Managed recovery, Selector verifies clean Git state and resumes WALK-001R and the complete guarded dry-run. The existing live workflow remains blocked until every guard and finite tier-gated Ambient proof pass.
