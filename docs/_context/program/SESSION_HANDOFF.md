...
WALK-002 Length Mode deletion is the sole ready Selector parcel. Delete `runLengthMode` from run intake, persisted constraints, readiness, summaries, spine/payload previews, option metadata and rendered UI. Preserve quantity and physical length, create no replacement or hidden default, and preserve all eligible NVB candidates for downstream Lex scoring. Gate and push, then stop before WALK-003. Seed execution remains blocked behind WALK-007; C6/C7, Engine, CRM/provider mutation, persistence, IES, retrieval and delivery remain blocked.
