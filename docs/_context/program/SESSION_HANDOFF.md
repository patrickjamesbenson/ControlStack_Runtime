...
WALK-002R remains the sole ready Selector parcel under an amended twelve-owner boundary. The second clean protected stop found the safe selected-result source projection still allowlisting and emitting `requested_length_basis`; add only that owner to the previous eleven. Remove all `runLengthMode`, `lengthMode`, `requested_length_basis` and `requestedLengthBasis` production occurrences, requirements, defaults, sentinels, fingerprints, proxies and interpretations; discard legacy mode input; preserve quantity and physical length only; preserve or broaden eligible NVB candidates for Lex scoring; gate and push, then stop before WALK-003. In the same parcel, delete or rewrite every test assertion and fixture expectation that encoded the assumption and report baseline, deleted, rewritten, affected-file and zero-residual counts.
