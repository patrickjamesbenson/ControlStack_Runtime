...
The deletion-test coupling rule applies to every later WALK deletion parcel: code deletion is incomplete while the test class survives. TEST-AUDIT-001 is recorded as a blocked future read-only parcel after WALK-007, covering deleted-input assertions, governance Engine preconditions, future/diagnostic prerequisites and identical-row fixture placeholder traps. Its output is a counted kill-list only and requires separate Program approval for any cleanup. Seed execution remains separately blocked; C6/C7, Engine, CRM/provider mutation, persistence, IES, retrieval and delivery remain blocked.
