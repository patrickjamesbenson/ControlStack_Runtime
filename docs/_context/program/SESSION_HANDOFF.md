...
WALK-004 is the sole ready Selector parcel under one writer. Exact production scope is `packages/workspace-kernel/selectorReferenceOptionsService.js` only. Delete the manual Tier field and all `TIERS` alias/debug/read/option/relationship authority. Preserve the unchanged 15-table `selectorReferenceService` readiness bridge, server/Engine `SYSTEM_POLICY` Tier derivation and only genuinely non-TIERS electrical-class options. Couple-delete or rewrite tests with a mechanical baseline and final residual zero; gate, push, leave the lane clean and stop before WALK-005.
