...
PWS-001, PWS-002 and PWS-005 are complete and are not reopened. WALK-001 remains open after a correctly fail-closed stop before all writes: Selector reports that the running service could not load the server-side Google Sheets reader. No Ambient/source validation, materialisation, archive or promotion is accepted. WALK-001R reader recovery and WALK-001 resume is the sole ready Selector parcel.

WALK-001R may reproduce the redacted failure and repair only the Selector-owned runtime dependency/module-resolution/configuration boundary or use existing managed-service controls. Credentials, Program deployment, protected-secret binding, other roots and broad environment repair remain out of scope. If those are required, stop again. Otherwise restart only `selector-runtime`, rerun the complete guarded dry-run and continue the existing live workflow only after every guard and the finite tier-gated Ambient proof pass.
