...
WALK-002R complete owner deletion is the sole ready Selector parcel. Its exact eleven-file production scope covers every current `runLengthMode`/`lengthMode` occurrence. Remove all mode requirements, fields, defaults, sentinels, fingerprints, proxies and interpretations; discard legacy mode input; preserve quantity and physical length only; preserve or broaden eligible NVB candidates for Lex scoring; require zero production occurrences before completion; gate and push, then stop before WALK-003. In the same parcel, delete or rewrite every test assertion and fixture expectation that encoded Length Mode and report baseline, deleted, rewritten, affected-file and zero-residual counts.
