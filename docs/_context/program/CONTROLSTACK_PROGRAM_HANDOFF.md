...
### WALK-003 accepted; WALK-004 legacy TIERS deletion released

Program accepts WALK-003. The duplicate expander scaffold is no longer projected or mounted; fabricated fresh-load selection values and their default-preview authority are absent from production and tests. Fresh state is empty-preamble, the genuine source-backed current-selection rail remains, the coupled-test receipt is 37 baseline / 0 deleted / 37 rewritten / 8 affected files / 0 residual, focused coverage passes 166/166, the fixed gate passes 108/108, and the lane is pushed and clean.

WALK-004 is the sole ready Selector parcel. Its exact production boundary is `packages/workspace-kernel/selectorReferenceOptionsService.js` only. Stop if another production owner is required.

Remove the legacy manual Tier field and all Selector reference-options ownership of the `TIERS` table: table aliases, safe-debug inclusion, live reads, emitted Tier options, TIERS-parented electrical-class options, source-table attribution and relationship records. Tier must not appear as a Selector manual input, selected value, option, payload/readiness prerequisite or table expectation.

Preserve `selectorReferenceService` unchanged as the sole source-readiness bridge. Its current 15 critical tables already exclude `TIERS`. Preserve the existing server/Engine derivation of Tier from `SYSTEM_POLICY`. Electrical Class may survive only from a genuine current non-TIERS source already supported by the service; otherwise expose it as unavailable without default, inference or copied legacy value.
...
### Sales Signals registration and SS-1G activation
