...
- **SELECTOR READY:** WALK-004 is the sole ready parcel under one writer. Exact production boundary: `selectorReferenceOptionsService.js` only; remove the legacy manual Tier/`TIERS` option and relationship authority while preserving the 15-table source-status bridge and server/Engine `SYSTEM_POLICY` Tier derivation.
...
- **NEXT:** remove the legacy manual Tier/`TIERS` option authority within the one-file production boundary, preserve the 15-table source-status bridge and server/Engine Tier derivation, couple-delete or rewrite tests with counted residual zero, run the fixed gate, push and stop for Program review. WALK-005 through WALK-007 remain sequence-blocked, with WALK-005A inserted before WALK-006. WALK-008 is ruled and closed.
