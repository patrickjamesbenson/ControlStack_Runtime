...
- **NEXT:** WALK-002R is accepted complete. WALK-003 duplicate-scaffold removal is the sole ready Selector parcel; no Lab action is ready and no seed artifact is generated.
...
- **SELECTOR READY:** WALK-003 is the sole ready parcel under one writer. Exact production boundary: `selectorState.js`, `selectorViewModel.js` and `selectorView.js`; remove the mounted single-page expander scaffold and fabricated module-local default current-selection authority while preserving the genuine source-backed current-selection rail.
...
- **PARALLELISM:** one Selector writer executes WALK-003 only. No Lab action is ready; MGR-RESTART-001 and both post-WALK audits remain blocked.
- **NEXT:** unmount the duplicate expander scaffold and remove its fabricated default current-selection ownership within the exact three-file production boundary, couple-delete or rewrite the tests encoding that authority with counted residual zero, preserve the source-backed live surface and no-write boundaries, run the fixed gate, push and stop for Program review. WALK-004 through WALK-007 remain sequence-blocked, with WALK-005A inserted before WALK-006. WALK-008 is ruled and closed.
