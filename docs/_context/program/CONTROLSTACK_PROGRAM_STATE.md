...
- **NEXT:** WALK-002RGT is accepted complete. The standing Selector worker resumes WALK-002R only; no Lab action is ready and no seed artifact is generated.
...
- **SELECTOR READY:** the existing standing worker resumes the same twelve-owner WALK-002R parcel. Baseline remains 71 encoded assumptions across 32 test files; all four banned-name zero searches and the final zero-residual rule remain binding.
