...
"""Host-local lumen curve CSV mirror runner.

The runner mirrors donor-owned static lumen curve CSV files into the RuntimeData
runtime authority-reference curve home. It is intentionally host-local: no web
surface is registered, no active snapshot is edited, and the manifest contains
checksums and public metadata only.
"""
...
SCHEMA_ID = "controlstack.runtime.authority-reference.lumen-curves-manifest"
SCHEMA_VERSION = 1
SOURCE_KIND = "lumen_curve_csv_static_mirror"
SOURCE_ROOT_CLASSIFICATION = "runtime-authority-reference"
SOURCE_CLASSIFICATION = "donor-static-mirror"

DEFAULT_SOURCE = Path(r"C:\ControlStack\data\lumen_curves")
DEFAULT_TARGET = Path(r"C:\ControlStack_RuntimeData\authority-reference\lumen_curves")
DEFAULT_MANIFEST = Path(r"C:\ControlStack_RuntimeData\authority-reference\lumen_curves_manifest.json")

UNSAFE_FILENAME_PATTERN = re.compile(r'[<>:"/\\|?*\x00]')
CSV_EXTENSION_PATTERN = re.compile(r"\.csv$", re.IGNORECASE)
...
def safe_filename(name: str) -> str:
    ...
    if not CSV_EXTENSION_PATTERN.search(filename):
        raise MirrorSafetyError("unsafe filename: not a CSV filename")
    return filename
...
def manifest_file_entry(source_file: SourceFile) -> dict[str, object]:
    return {
        ...
        "relative_path": f"lumen_curves/{source_file.filename}",
...
def validate_manifest_shape(manifest: dict[str, object]) -> None:
    ...
    if manifest.get("raw_curve_rows_included") is not False:
        raise MirrorSafetyError("manifest raw row guard failed")
    ...
    for entry in files:
        ...
        if entry.get("relative_path") != f"lumen_curves/{filename}":
...
def validate_roots(source: Path, target: Path, manifest_path: Path, allow_test_path_overrides: bool) -> None:
    if not allow_test_path_overrides:
        if not same_path(source, DEFAULT_SOURCE):
            raise MirrorSafetyError("source root must be the donor lumen_curves home")
        if not same_path(target, DEFAULT_TARGET):
            raise MirrorSafetyError("target root must be the RuntimeData authority-reference lumen_curves home")
        if not same_path(manifest_path, DEFAULT_MANIFEST):
            raise MirrorSafetyError("manifest path must be the RuntimeData authority-reference lumen_curves manifest")
    ...
    if manifest_path.name != "lumen_curves_manifest.json":
        raise MirrorSafetyError("manifest filename must be lumen_curves_manifest.json")


def discover_csv_files(source: Path, target: Path) -> tuple[list[SourceFile], list[str], list[dict[str, str]]]:
    ...
    for entry in sorted(source.iterdir(), key=lambda item: item.name.casefold()):
        if not entry.is_file():
            ...
            continue
        if not CSV_EXTENSION_PATTERN.search(entry.name):
            ignored.append(entry.name)
            continue
...
def run_mirror(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    source_files, ignored, conflicts = discover_csv_files(source, target)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Mirror donor lumen curve CSVs into RuntimeData authority-reference storage.")
    parser.add_argument("--source", default=str(DEFAULT_SOURCE), help="Donor lumen_curves source directory.")
    parser.add_argument("--target", default=str(DEFAULT_TARGET), help="RuntimeData authority-reference lumen_curves target directory.")
    parser.add_argument("--manifest", default=str(DEFAULT_MANIFEST), help="RuntimeData lumen_curves manifest path.")
